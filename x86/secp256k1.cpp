...
uint8_t Secp256K1::GetByte(std::string &str, int idx) {
  ...
  if (tmp[0] == '0' && tmp[1] == 'x') {
    tmp[0] = str.data()[2 * idx + 2];
    tmp[1] = str.data()[2 * idx + 3];
  }
...
// Add Affine + Affine
Point Secp256K1::AddAffine(Point &p1,Point &p2) {
  Int _s;
  Int _p;
  Int dy;
  Int dx;
...
// Add mixed Jacobian + Affine
Point Secp256K1::AddMixed(Point &p1, Point &p2) {
  Int u;
  Int v;
  Int u1;
  Int v1;
...
Point Secp256K1::DoubleAffine(Point &p) {
  Int _s;
  Int _p;
  Int a;
  Point r;
...
std::vector<Point> Secp256K1::BatchComputePublicKeys(Secp256K1 *s,
                                         std::vector<Int> &privKeys) {
  std::vector<Point> points;

  // Compute projective coordinates.
  for (Int i : privKeys) {
    points.push_back(s->ComputePublicKey(&i, false));
  }
  ...
  // Invert z values.
  IntGroup group(points.size());
  group.Set(z);
  group.ModInv();

  // Multiply points by inverted z values.
  for (int i = 0; i < points.size(); i++) {
    points[i].x.ModMul(&points[i].x, &z[i]);
    points[i].y.ModMul(&points[i].y, &z[i]);
