...
void Int::Rand(int nbit, unsigned char *seed) {
  CLEAR();
  ...
  // Use provided seed bytes
  for (; i < nb && i * 4 < 256; i++) {
    uint32_t val = 0;
    val |= ((uint32_t)seed[i * 4]) << 24;
    val |= ((uint32_t)seed[i * 4 + 1]) << 16;
    val |= ((uint32_t)seed[i * 4 + 2]) << 8;
    val |= ((uint32_t)seed[i * 4 + 3]);
    bits[i] = val;
  }
...
void Int::Rand(Int *randMax, unsigned char *seed) {
...
int Int::SetBase16(char *value) { return SetBaseN(16, "0123456789ABCDEF", value); }
...
int Int::SetBaseN(int n, char *charset, char *value) {
