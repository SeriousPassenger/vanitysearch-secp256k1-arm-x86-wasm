...
// Generic implementations for ARM using 128-bit arithmetic
...
static inline unsigned char _addcarry_u64(unsigned char c, uint64_t a, uint64_t b, uint64_t *res) {
  unsigned __int128 sum = (unsigned __int128)a + b + c;
  *res = (uint64_t)sum;
  return (unsigned char)(sum >> 64);
}

static inline unsigned char _subborrow_u64(unsigned char c, uint64_t a, uint64_t b, uint64_t *res) {
